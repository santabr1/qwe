...
function getDevelopersManager(err, res, req, result, errMessage) {
    ...
    if (Array.isArray(result)) {
        for (let developer of result) {
            ...
            developer.developer_avatar_url = developer.developer_avatar_url || `${config.BACK_ADDRESS}/images/avatar.png`
...
developerRouter.post('/put-avatar/:id', upload.single('image'), (req, res) => {
    try {
        ...
        const query = sqlSafeDecorator(developersQueries.changeAvatarById, id, `${config.BACK_ADDRESS}/` + imagePath)()
